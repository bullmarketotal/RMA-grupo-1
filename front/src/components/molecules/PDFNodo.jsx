...
const PDFNodo = ({
  ...
  useEffect(() => {
    const capturePDF = async () => {
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      const descripcionLines = splitText(
        data.descripcion,
        pageWidth - marginX * 2
      );
      ...
      doc.setFont("helvetica", "normal");
      const hoy = new Date();
      const introductionLines = splitText(
        `Informe generado para el nodo ${data.identificador} (Nº ${data.id}) al ${hoy.getDate()} de ${mesesEsp[hoy.getMonth()]} del ${hoy.getFullYear()}.`,
        pageWidth - marginX * 2
      );
      ...
      if (bateriaChartRef.current) {
        ...
        const originalCanvas = await html2canvas(bateriaChartRef.current, {
          scale: 2,
        });
  ...
  return (
    <button onClick={onExport} className="btn-active btn-action mt-4">
      Exportar a PDF
    </button>
  );
};
